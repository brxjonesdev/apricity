...
export async function reorderChapter(storyId: string, chapterIds: string[]): Promise<Chapter[]>{
  ...
  const res = await call<ChapterDTO[]>("reorder_chapters", { story_id: storyId, new_order: chapterIds });
  if (!res.ok) {
